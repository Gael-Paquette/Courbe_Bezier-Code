...
def trace_ligne_brisee(points):
    ...
    for i in range(len(points)):
        if i > 0:
            ...
            xB = points[i][0]
            yB = points[i][1]
            ligne(xA, yA, xB, yB, couleur='red', epaisseur=1, tag='')
...
def calculs_milieu_suites_points(points):
    ...
    for i in range(len(points)):
        if i > 0:
            A = points[i-1]
            B = points[i]
            liste_milieu.append(milieu(A, B))                          
    return liste_milieu
...
def etape_casteljau(points):
    ...
    * le point P_i^j, si 0 < j <= N, est le milieu du segement reliant P_i^(j-1) à P_(i+1)^(j-1)
    ...
    liste_violette = []
    liste_milieu = points
    liste_rouge.append(points[0])
    liste_violette.append(points[len(points)-1])
    for i in range(len(points)-1):         
        liste_milieu = calculs_milieu_suites_points(liste_milieu)
        liste_rouge.append(liste_milieu[0])
        if len(liste_milieu) != 1:
            liste_violette.append(liste_milieu[len(liste_milieu)-1])
    liste_violette.reverse()
    return liste_rouge, liste_violette
...
def bezier_casteljau(points):
    ...
    if len(points) > 1:
        if distance_max(points) > 1:
            liste_rouge, liste_violette = etape_casteljau(points)
            bezier_casteljau(liste_rouge + liste_violette)
        else:
            for i in range(len(points)):
                if i > 0:
                    A = points[i-1]
                    B = points[i]
                    trace_ligne_brisee([A,B])
